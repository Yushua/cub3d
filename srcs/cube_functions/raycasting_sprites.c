...
int         render_next_frame_sprites(t_struct_m *main)
{
	...
	while (i < main->numSprites)
	{
		...
		main->spriteDistance[i] = ((main->Ray.posX - main->sprites[i].x) * (main->Ray.posY - main->sprites[i].x) +
		(main->Ray.posY - main->sprites[i].y) * (main->Ray.posY - main->sprites[i].y));
		i++;
	}
	...
	while (i < main->numSprites)
	{
		...
		main->sprite.invDet = 1.0 / (main->Ray.planeX * main->sprite.spritex - main->Ray.dirX * main->Ray.planeY);
		...
		main->sprite.spriteHeight = abs((int)(main->place.s_height / (main->sprite.transformY))) / vDiv;//using "transformY" instead of the real distance prevents fisheye
		...
		main->sprite.spriteWidth = abs((int)(main->place.s_height / (main->sprite.transformY))) / uDiv;
		...
		while (main->sprite.stripe < main->sprite.drawendX)
		{
			...
			if (main->sprite.transformY > 0 && main->sprite.stripe > 0 && main->sprite.stripe < main->place.s_width && main->sprite.transformY < main->ZBuffer[main->sprite.stripe])
			{
				...
				{
					int		d;
					d = (y - main->sprite.vMoveScreen) * 256 - main->place.s_height * 128 + main->sprite.spriteHeight * 128;
					int		texY;
					int		colour;
